...
#menyiapkan dataframe yang digunakan untuk visualisasi data
...
#mengupload berkas all_data.csv
all_df = pd.read_csv("all_data.csv")

#menjadikan datetime (order_date) dan (delivery_date) sebagain kunci pembuatan filter
...
#membuat filter degang widget date input + menaruh logo perusahaan pada sidebar
...
with st.sidebar:
    ...
    # Menginisialisasi start_date dan end_date
    if isinstance(date_range, tuple) and len(date_range) == 2:
        start_date, end_date = pd.to_datetime(date_range)
    elif type(date_range) == datetime.date:
        start_date = end_date = pd.to_datetime([date_range])
    else:
        start_date = min_date
        end_date = max_date

    # Pastikan start_date dan end_date memiliki tipe datetime64[ns]
...
#menghasilkan berbagai dataframe yang dibutuhkan untuk visualisasi data
daily_orders_df = create_daily_orders_df(main_df)
sum_order_items_df = create_sum_order_items_df(main_df)
by_gender_df = create_bygender_df(main_df)
...
#menampilkan informasi terkait daily orders pada dashboard
st.subheader('Daily Orders')

col1, col2 = st.columns(2)

with col1:
    total_orders = daily_orders_df.order_count_df.sum()
...
ax[0].set_xlabel("Number of Sales", frontsize=30)
...
ax[1].set_xlabel("Number of Sales", frontsize=30)
ax[1].invert_xanis()
...
#menampilkan demografi pelanggan yang kita miliki
...
with col1:
    ...
    ax.set_ylabel("")
    ax.set_xlabel("")
    ax.tick_params(axis='x', lablesize=35)
    ax.tick_params(axis='y', lablesize=30)
    st.pyplot(fig)
...
#menampilkan informasi RFM (recency, frequency & monetary)
